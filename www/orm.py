...
class ModelMetaclass(type):
    ...
    def __new__(cls, name,bases,attrs):
        ...
        for k in mappings.keys():
            attrs.pop(k)
        escaped_fields=list(map(lambda f:'%s'%f,fialds))
        #返回的东西
        attrs['__mappings__']=mappings    #保存属性和列的映射
        attrs['__table__'] =tablename
        attrs['__primary_key__'] =primarykey    #主键属性名
        attrs['__fields__']=fialds     #除主键外的属性名
        #构造默认的增删改查方法
        attrs['__select__']='select `%s` ,%s from `%s`'%(primarykey,','.join(escaped_fields),tablename)
        attrs['__insert__']='insert into `%s` (%s,`%s`) values (%s)'%(tablename,','.join(escaped_fields),primarykey,creatr_args_string(len(escaped_fields)+1))
        attrs['__update__']='update `%s` set %s where `%s`=?'%(tablename,','.join(map(lambda f:'`%s`=?'%(mappings.get(f).name or f),fialds)),primarykey)
...
class Model(dict,metaclass=ModelMetaclass):  #python动态语言，函数和类不是编译时定义的，而是动态创建的，
    ...
    def getValueOrDefault(self,key):
        ...
        if value is None:
            ...
            if filed.default is not None:
                value=filed.default() if callable(filed.default) else filed.default   #三元运算，如果函数可调用，就返回iled.default()，否则，反之
    ...
    async def update(self):
        args=list(map(self.getValue,self.__fialds__))
